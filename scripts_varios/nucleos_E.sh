...
## Script antiguo para lanzar las aplicacioines en los núcleos E sin el Manager
## Daniel Mirón
...
function wait_for_free_core() {
    while true; do
        ...
        echo "Todos los núcleos E ocupados. Esperando liberación..."
        sleep 10
    done
}
...
##cpupower -c 16-31 frequency-set -f 2200000    # Núcleo E
...
while read WL; do
        ...
        sudo taskset -c ${ASSIGNED_CORE} /home/dmirvid/repo_tfg/launch.sh ${WL[@]} ${ASSIGNED_CORE} ${EVENTS_E} > out 2> err &
        app_pid=$!
...
echo "Todas las apps ejecutadas en núcleos E"
