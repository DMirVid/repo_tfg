## Política de asignación de cores basado en el Backend Bound
## Intento de que aplicaciones con bajo IPC se intercalaran segun su Bacend Bound.
## NO salió como esperaba.
## Daniel Mirón

from config import CPUS, QUANTUM_SIZE, INSTRUCTION_COUNT_P, INSTRUCTION_COUNT_E, CYCLE_COUNT_P, CYCLE_COUNT_E, BACKEND_BOUND, BACKEND_BOUND_E
import results

# Events used by the policy
EVENTS = [INSTRUCTION_COUNT_P, INSTRUCTION_COUNT_E, CYCLE_COUNT_P, CYCLE_COUNT_E, BACKEND_BOUND, BACKEND_BOUND_E]
    ...
    for i, proc in enumerate(processes):
        ...
        try:
            if es_P:
                ...
                prio[i] = proc.event_counts[BACKEND_BOUND] / (proc.event_counts[CYCLE_COUNT_P] * 6)

            else:
                ipc[i] = proc.event_counts[INSTRUCTION_COUNT_E] / proc.event_counts[CYCLE_COUNT_E]
                prio[i] = proc.event_counts[BACKEND_BOUND_E] / (proc.event_counts[CYCLE_COUNT_E] * 5)

        except ZeroDivisionError:
            #vlaores por defecto en caso de error
            prio[i] = 0
            ipc[i] = 1


    sorted_procs = sorted(processes, key=lambda proc: ipc[processes.index(proc)])
    sorted_procs[4:-1] = sorted(sorted_procs[4:-1], key=lambda proc: prio[processes.index(proc)])
    
    # Intercalar prioridades en los núcleos E
    tmp = sorted_procs[5]
    sorted_procs[5] = sorted_procs[-1]
    sorted_procs[-1] = tmp
