...
def main():
    # Configurar fuente a Times New Roman
    plt.rcParams['font.family'] = 'Times New Roman'

    if len(sys.argv) < 3:
        print("Uso: python graficar.py P_cores.csv E_cores.csv")
        return
    ...
    for line_P, line_E in zip(f_p, f_e):
        ...
        # Solo procesar si la aplicación está en la mezcla
        #if datos_P[0] not in mezcla_list:
         #   continue
        ...
        retiring_values.append((retiring_P / slots_P, speedup_tiempo * retiring_E / slots_E))
        bad_speculation_values.append((bad_spec_P / slots_P, speedup_tiempo * bad_spec_E / slots_E))
        frontend_values.append((fe_bound_P / slots_P, speedup_tiempo * fe_bound_E / slots_E))
        backend_bound.append((be_bound_P / slots_P, speedup_tiempo * be_bound_E / slots_E))
    ...
    # Ordenar por el orden de la tupla mezcla
    # from collections import defaultdict
    # mezcla_indices = defaultdict(list)
    # for idx, app in enumerate(mezcla_list):
    #     mezcla_indices[app].append(idx)
    
    # # Rastrear qué ocurrencia de cada app hemos visto
    # app_occurrence = defaultdict(int)
    # sort_indices = []
    
    # for app in app_names:
    #     occurrence = app_occurrence[app]
    #     if occurrence < len(mezcla_indices[app]):
    #         sort_indices.append(mezcla_indices[app][occurrence])
    #     app_occurrence[app] += 1
    
    # sorted_indices = sorted(range(len(sort_indices)), key=lambda i: sort_indices[i])
    ...
    color_map = ["#2E75B6", "#F4B183", "#EBCC0F", "#C00000"]
    ...
    for i in range(len(app_names)):
        ...
        p42 = ax1.bar(x_pos[i] - width/4, backend_bound[i][0], width/2, bottom=cumulative_P,
        ...
        ax1.set_xticklabels(x_labels, fontsize=20, rotation=90, fontfamily='Times New Roman')
        ax1.set_ylabel('Porcentaje de tiempo de ejecución', fontsize=20, fontfamily='Times New Roman')
        ...
        ax1.legend(lines1 , labels1, loc='center', bbox_to_anchor=(0.5, 1.15), fontsize=20, ncol=7, frameon=True, prop={'family': 'Times New Roman', 'size': 20})
